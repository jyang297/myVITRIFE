...
def train(model, local_rank):
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch + 1) % 20 == 0:  # Adding 1 to make it human-readable (epochs start from 1)
...
if __name__ == "__main__":    
    ...
    parser.add_argument("--checkpoint", type=str, help='path to the pre-trained model checkpoint', default="Or_train_log/flownet.pkl")
...
#
    ...
    if args.checkpoint == 'a':
        if os.path.isfile(args.checkpoint):
            ...
            # Ensure the map_location is set to the target device
            checkpoint = torch.load(args.checkpoint, map_location=device)
            # Assuming your checkpoint structure has 'flownet' key with the state dict
            model.load_state_dict(checkpoint['flownet'])
